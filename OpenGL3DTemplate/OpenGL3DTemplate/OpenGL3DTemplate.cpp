...
int lastTickMs=0;
// object collision radii (horizontal) at platform centers (moved earlier for initScene)
...
void initScene(){
    ...
    for(int i=0;i<4;i++){
        ...
        for(int c=0;c<count;c++){
            // rejection sampling to keep coins away from central object radius
...
void MouseMotion(int x,int y){
    if(gameOver) return; // disable after losing
    if(currentView==FREE_VIEW && freeMouseRotate){
        int dx = x - lastMouseX;
        int dy = y - lastMouseY;
